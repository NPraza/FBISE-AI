...
<a-row class="border-top">
    <a-divider />
    <a-col :span="8" class="ocr-image">
        <img :src="ocr_data.image_src">
        <p>{{ ocr_data.file_name }}</p>
    </a-col>
    <a-col :span="10" class="text-center"><a-textarea v-model:value="ocr_data.ocr_result" @focusout="updateResult(ocr_data.ocr_result_id, { answer: ocr_data.ocr_result })" placeholder="Basic usage" :rows="7" /></a-col>
    <a-col :span="6" class="text-center"> 
        <div class="action-btn-wrapper">
        <button :class="`action-btn ${ocr_data.feedback == 1 ? 'fill' : ''}`" @click="updateFeedbackOnOcr(1, ocr_data)">
        ...
        <button :class="`action-btn ${ocr_data.feedback == 5 ? 'fill' : ''}`" @click="updateFeedbackOnOcr(5, ocr_data)">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
